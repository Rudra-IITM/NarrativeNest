...
import { SignUpInput } from '@rudra_iitm/web-blog'
...
const SignupAuth = () => {
  ...
  const [postInfo, setPostInfo] = useState<SignUpInput>({
    email: "",
    password: "",
    name: ""
  })
  ...
  const signup = async () => {
    try {
      const { data } = await axios.post(`${BACKEND_URL}/api/v1/user/signup`, postInfo)
